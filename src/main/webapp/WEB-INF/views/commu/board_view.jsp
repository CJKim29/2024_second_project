				...
				<div class="panel panel-primary">
				      ...
				      <div class="panel-body">
				      	...
				      	<div>
				      		...
				      		<!-- 로그인된 상태 또는 메인글인 경우에만 답글쓰기 가능 -->
				      		<c:if test="${ not empty user and (vo.b_depth eq 0) }">
				      			<input class="btn btn-success" type="button" value="답글쓰기" onclick="location.href='reply_form.do?b_idx=${vo.b_idx}'"> 
				      		</c:if>
				      		
				      		<!-- 글의 주인만 수정/삭제 가능 -->
				      		<c:if test="${ vo.mem_idx eq user.mem_idx }">
					      		<input class="btn btn-info" type="button" value="수정하기" onclick="location.href='modify_form.do?b_idx=${vo.b_idx}'">
					      		<input class="btn btn-danger" type="button" value="삭제하기" onclick="del();">
				      		</c:if>
				      	</div>
				      </div>
				</div>

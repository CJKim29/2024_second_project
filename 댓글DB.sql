/*
	...
	-- 외래키
	alter table comment_tb
		add constraint comment_tb_b_idx foreign key(b_idx)
										references board(b_idx)
										
	alter table comment_tb
		add constraint fk_comment_tb_mem_idx foreign key(mem_idx) references member(mem_idx)
	...
	(
		...
		from
		(
			select * from comment_tb where b_idx=8
		) c
	)
